...
async function getFairlaunchSubmissions() {
  ...
  const { data: rounds, error } = await supabase
    ...
    .in('status', ['SUBMITTED', 'APPROVED', 'REJECTED', 'DEPLOYED'])
...
export default async function FairlaunchReviewPage() {
  ...
  const pending = rounds.filter((r) => ['SUBMITTED'].includes(r.status));
  const reviewed = rounds.filter((r) => ['APPROVED', 'REJECTED', 'DEPLOYED'].includes(r.status));
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-gray-300 capitalize">
                        {round.chain || round.chain_id}
                      </td>
                      <td className="p-4 text-gray-300">{round.params?.softcap || 'N/A'}</td>
                      <td className="p-4">
