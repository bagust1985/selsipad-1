...
export function AgoraVoiceRoom({
  ...
  useEffect(() => {
    const init = async () => {
      try {
        console.log('[Agora Voice] Initializing...', { channelName, uid, userRole });

        // Create Agora client - use 'live' mode for broadcasting
        ...
        // Set role
        await client.setClientRole(userRole === 'publisher' ? 'host' : 'audience');

        // Event listeners
        client.on('user-published', async (user, mediaType) => {
          console.log('[Agora Voice] User published:', user.uid, mediaType);
          await client.subscribe(user, mediaType);
          if (mediaType === 'audio') {
            console.log('[Agora Voice] Playing remote audio from user:', user.uid);
        ...
        client.on('user-unpublished', (user, mediaType) => {
          console.log('[Agora Voice] User unpublished:', user.uid, mediaType);
          setParticipants((prev) => Math.max(1, prev - 1));
        });

        client.on('user-left', (user) => {
          console.log('[Agora Voice] User left:', user.uid);
          setParticipants((prev) => Math.max(1, prev - 1));
        });

        client.on('user-joined', (user) => {
          console.log('[Agora Voice] User joined:', user.uid);
        });

        // Join channel with the uid from token
        console.log('[Agora Voice] Joining channel:', channelName, 'with uid:', uid);
        await client.join(appId, channelName, token, uid);
        console.log('[Agora Voice] ✅ Joined successfully');
        setIsJoined(true);

        // Create and publish audio track if publisher
        if (userRole === 'publisher') {
          console.log('[Agora Voice] Creating microphone track...');
          const audioTrack = await AgoraRTC.createMicrophoneAudioTrack({
            encoderConfig: 'high_quality_stereo',
          });
          audioTrackRef.current = audioTrack;
          await client.publish([audioTrack]);
          console.log('[Agora Voice] ✅ Audio track published');
        }
  ...
  }, [amaId, channelName, token, appId, uid, userRole]);
