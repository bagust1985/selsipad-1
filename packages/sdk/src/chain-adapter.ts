...
export class EVMAdapter implements IChainAdapter {
  ...
  async waitForFinality(txHash: TxHash, _confirmations = 12): Promise<TxReceipt> {
  ...
  async getTxReceipt(_txHash: TxHash): Promise<TxReceipt | null> {
...
export class SolanaAdapter implements IChainAdapter {
  ...
  async waitForFinality(txHash: TxHash, _confirmations = 32): Promise<TxReceipt> {
